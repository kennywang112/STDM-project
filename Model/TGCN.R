...
model_tgcn$load_state_dict(torch_load("./Data/CalculatedData/best_model_tgcn.pt"))
model_tgcn$to(device = device)
model_tgcn$eval()
with_no_grad({
  pred_tgcn <- as.numeric(model_tgcn(x_test_t_tensor_3d$to(device = device), A_dense)$to(device = "cpu"))
})

test_results_tgcn <- test %>%
  mutate(
    real_actual = accident_count * (acc_max - acc_min) + acc_min,
    Predicted_ann = pred_ann * (acc_max - acc_min) + acc_min,
    Predicted_tgcn = pred_tgcn * (acc_max - acc_min) + acc_min,
    mse_ann = (real_actual - Predicted_ann)^2,
    mse_tgcn = (real_actual - Predicted_tgcn)^2
  ) %>%
  mutate(accident_count = real_actual) %>%
  select(-real_actual)

test_results_tgcn %>% write.csv("./Data/CalculatedData/test_results_tgcn.csv", row.names = FALSE)
test_results_tgcn <- read.csv("./Data/CalculatedData/test_results_tgcn.csv")
