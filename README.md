...
## Results
All results are stored on [google drive](https://drive.google.com/drive/folders/13MidSKUTYiyJUXR11ds-ek9NKnwBIK1T?usp=drive_link) with results plot, all models, and the data for running the code.

## Code Files
1.  `ST_Full.R` is used to construct the data for all models, it run models from `STSVR.R`, `STARIMA.R`, `STGCN.R`, `TGCN.R`, and `LSTMGNN.R`
...
4. `PlotNNCurve.R` is implementing the learning curve for all NN models.
5. `SpatialFeatureMap.R` is to plot the spatial features, more specifically different road type mapping at once.
6. `Model_EDA.qmd` is the final analysis includign residual map, total mse, comparison between predicted values and ground truth.