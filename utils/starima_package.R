...
#the program to estimate space-time autocorrelation function (STACF)
  #Inputs:
  ##Z: The space-time series (T*N) where T is time length and N is number of locations
  ##W: The spatial weight matrix
  ##nLags: The number of lags for which to calculate the STACF
...
# Compute the space time autocovariance
	...
	stacf[s+1,1] <- (Rlk/sqrt(Rll*Rkk))*(T/(T-s));
#	stacf[s,1] <- (Rlk/sqrt(Rll*Rkk))*(T/(T-s));
}

# Calculate confidence intervals

nSE <- 2
sigmaQ <- sqrt(1/((T-nLags)*N));
ubound <- sigmaQ*nSE;
lbound <- sigmaQ*-nSE;

stacfP <- cbind(c(0:(nrow(stacf)-1)), stacf)
plot(stacfP, main="Space-Time Autocorrelation Function",xlab="Time Lags", ylab="ST-Autocorrelation", type="h", col="red", ylim=c(min(stacf), max(stacf)));
...
lenB <-length(model$BETA);

partialSTACF[order,1]<-model$BETA[lenB];
...
plot(partialSTACF, main="Space-Time Partial Autocorrelation Function",xlab="Time Lags", ylab="ST Partial Autocorrelation", type="h", col="red", ylim=c(min(partialSTACF), max(partialSTACF)));
...
znorm <- function (Z=NULL)
{
  ...
  # A function to normalize a vector of observations 
...
myPacf <- function(x, k)
{
  ## A function to calculate the partial autocorrelation 
  ## up to lag k.
  #Inputs:
  # x: a time series
  # k: maximum lag
  
  ## Calculate the acf up to lag k
  
  ACF <- acf(x, k)
  
  ## Build the coefficient matrix R.
  
  R <- matrix(0, k, k)
  
  for(i in 1:k)
  {
    R[i,i:k] <- ACF$acf[1:((k+1)-i)]
  }
  R <- R+t(R)-diag(k)
  
  ## Create the vector r
  
  r <- matrix(ACF$acf[2:(k+1)], , 1)
  
  ## Create a vector to store the partical autocorrelations
  
  p <- matrix(0, k, 1)
  
  ## Solve the regression equations for lags 1 to k.
